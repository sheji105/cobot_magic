...
class DiffusionPolicy(nn.Module):
    ...
    def __call__(self, image, depth_image, robot_state, next_actions, next_actions_is_pad, actions=None,
        ...
        if actions is not None:
            noise, noise_pred = self.model(image, depth_image, robot_state, next_actions, next_actions_is_pad, actions, action_is_pad)
            # L2 loss
            ...
            return self.model(image, depth_image, robot_state, next_actions, next_actions_is_pad, actions, action_is_pad)
    ...
    def __call__(self, image, depth_image, robot_state, next_actions, next_actions_is_pad, actions=None,
        ...
        if actions is not None:  # training time
            ...
            a_hat, (_, _) = self.model(image, depth_image, robot_state, next_actions,
                                       next_actions_is_pad)  # no action, sample from prior
            return a_hat
