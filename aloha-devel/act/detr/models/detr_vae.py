...
class DETRVAE(nn.Module):
    ...
    def forward(self, image, depth_image, robot_state, next_actions, next_action_is_pad, actions=None, action_is_pad=None):
        """
        qpos: batch, qpos_dim
...
class CNNMLP(nn.Module):
    def __init__(self, backbones, depth_backbones, state_dim, camera_names, num_next_action):
        """ Initializes the model.
        Parameters:
            backbones: torch module of the backbone to be used. See backbone.py
            transformer: torch module of the transformer architecture. See transformer.py
        ...
        if backbones is not None:
            ...
            mlp_in_dim = 768 * len(backbones) + state_dim + state_dim * self.num_next_action
    ...
    def forward(self, image, depth_image, robot_state, next_actions, next_action_is_pad, actions=None, action_is_pad=None):
        ...
        if self.num_next_action != 0 and next_actions is not None:
            features = torch.cat([flattened_features, robot_state, next_actions], axis=1)  # qpos: 14
        else:
...
class Diffusion(nn.Module):
    def __init__(self, backbones, pools, linears, depth_backbones, state_dim, chunk_size,
                 ...
                 ema_power, camera_names, num_next_action):
        """ Initializes the model.
        Parameters:
            backbones: torch module of the backbone to be used. See backbone.py
            transformer: torch module of the transformer architecture. See transformer.py
        ...
        self.noise_pred_net = ConditionalUnet1D(
            ...
            global_cond_dim=self.obs_dim * self.observation_horizon + state_dim * self.num_next_action
        )
        if depth_backbones is not None:
            nets = nn.ModuleDict({
    ...
    def forward(self, image, depth_image, robot_state, next_actions, next_action_is_pad, actions=None,
                action_is_pad=None):
